...
object Sort {
  ...
  val all: Vector[Sort] = Vector(NextShowing, ImdbRating, TmdbRating, TmdbVotes)
  ...
  case object TmdbVotes extends Sort("votes", "TheMovieDatabase Votes (Descending)", Ordering.by { e: Entry => e._1.tmdbVotes.getOrElse(0) }.reverse)
}
