...
object DatabaseInitialisation {
  ...
  def createCinemas(db: DB): Future[Unit] =
    for {
      _ <- db.run(sqlu"create table if not exists cinemas (key integer, json varchar not null, primary key (key)); ")
      _ <- db.run(sqlu"insert into cinemas(key, json) values (1, '') on conflict do nothing")
    } yield ()
